...
vec4 calcSpotLight(SpotLight light, vec3 normal) {
    ...
    if(spotFactor > light.cutoff) {
        color = calcPointLight(light.pointLight, normal) * (1 - (1-spotFactor)/(1-light.cutoff));
    }
...
void main() {
    oColor = texture(uSampler, fTexCoord) * calcSpotLight(uSpotLight, normalize(fNormal));
}